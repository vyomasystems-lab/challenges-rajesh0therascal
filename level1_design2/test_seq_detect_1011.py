...
async def test_seq_op_same_cycle(dut):
    ...
    for bit in range(len(bitString)):
        if bitString[bit] == '1':
            ...
            dut.inp_bit.value = 0
        #dut._log.info(f'Bit {bit} arrived and current state is {int(dut.current_state)}')
        await FallingEdge(dut.clk)
        if (dut.seq_seen == 1):
...
async def test_seq_all_bruteforce(dut):
    ...
    for inputs in range(8,128):
        ...
        for bit in range(len(bitString)):
            ...
            if (dut.seq_seen == 1):
                testCount = testCount + 1
                #dut._log.info(f'In current cycle current state is {int(dut.current_state)}')
                #dut._log.info(f'Pulse detected at wrong location')
        #dut.inp_bit.value = 0
        #await FallingEdge(dut.clk)
        #if(dut.seq_seen == 1):
            #testCount = testCount + 1
            #dut._log.info(f'In next cycle cycle current state is {int(dut.current_state)}')
            #dut._log.info(f'Pulse detected at right location')
            
        #dut._log.info(f'Testcount is {testCount}')
